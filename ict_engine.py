...
class ICTEngine:
    ...
    def compute_smc_features(self, df):
        ...
        df_work.columns = [col.lower() for col in df_work.columns]
        
        # Fair Value Gaps
        fvg = smc.fvg(df_work)
        
        # Swing Highs/Lows
        swing_hl = smc.swing_highs_lows(df_work, swing_length=50)
        
        # Order Blocks
        ob = smc.ob(df_work, swing_hl)
        
        # BOS and CHoCH
        bos_choch = smc.bos_choch(df_work, swing_hl)
        
        # Liquidity
        liquidity = smc.liquidity(df_work, swing_hl)
    ...
    def get_bias(self, df):
        ...
        swing_hl = smc.swing_highs_lows(df_work, swing_length=20)
        bos_choch = smc.bos_choch(df_work, swing_hl)
